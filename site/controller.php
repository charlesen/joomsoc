...
class JoomSocController extends JControllerLegacy {
	...
	function status() {		
		...
		try {
			...
			if ( $user->id ) {
				...
				if ( $data->id ) {
					$data->user = $user;
					$data->assetPath = JURI::base(true). '/components/com_joomsoc/assets';
	...
	function upload() {
		...
		try {
			...
			// Upload file
			$src = $photo['tmp_name'];
			$dest = "/components/com_joomsoc/assets/uploads/".$userId."/photos/".$filename;
